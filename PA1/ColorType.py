...
class ColorType:
    ...
    def __mul__(self, scale: int):
        """
        For quick color interpolation stuff
        """
        return ColorType(self.r*scale, self.b*scale, self.g*scale)

    def __add__(self, other):
        if not isinstance(other, ColorType):
            raise TypeError('Can only add ColorType object with ColorType object')
...
RED = ColorType(1, 0, 0)
GARFIELD_ORANGE = ColorType(245/255, 192/255, 19/255)
