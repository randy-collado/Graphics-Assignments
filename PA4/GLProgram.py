...
class GLProgram:
    ...
    def __init__(self) -> None:
        ...
        self.attribs["ambientOn"] = self.attribs["material"] + ".ambientOn"
        self.attribs["diffuseOn"] = self.attribs["material"] + ".diffuseOn"
        self.attribs["specularOn"] = self.attribs["material"] + ".specularOn"
        ...
        {{
            ...
            if ((renderingFlag >> 0 & 0x1) == 1){{
                ...
                for (int i = 0; i < MAX_LIGHT_NUM; i++){{
                    ...
                    //Normal calculations for diffuse light
                    
                    float _dp = max(dot(light_vector, vNormal), 0);
                    float _dp_spot = max(dot({self.attribs["light"]}[i].spotDirection, vNormal), 0);
                    float _dp_inf = max(dot({self.attribs["light"]}[i].infiniteDirection, vNormal), 0);

                    //Diffuse light not complete
                    vec4 diffuse_light;
                    diffuse_light = {self.attribs["material"]}.diffuse*{self.attribs["light"]}[i].color*_dp;

                    

                    vec4 spec_light_vec = {self.attribs["material"]}.specular*{self.attribs["light"]}[i].color;
                    vec3 r_vec;
                    r_vec = (2*_dp*vNormal) - light_vector;
                    
                    float _dp_spec = max(dot(vPos, r_vec), 0);
                    vec4 spec_light = spec_light_vec*pow(_dp_spec, {self.attribs["material"]}.highlight);
                    result += ambient_light + diffuse_light + spec_light;
                    
                }}
                ////////// TODO 3: Illuminate your meshes - Complete
                // Requirements:
            ...
            if ((renderingFlag >> 3 & 0x1) == 1){{
                ...
                vec3 offsetNormal = vNormal + offset;
                vec3 offsetNormNormalized = vec3(offsetNormal.x/2, offsetNormal.y/2, offsetNormal.z/2);
                results[ri] = vec4(offsetNormNormalized.x, offsetNormNormalized.y, offsetNormNormalized.z, 1.0);
                ri+=1;
            }}
