...
class Sketch(CanvasBase):
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        if keycode in [wx.WXK_LEFT]:
            self.nextScene(forward=False)
            self.update()
        if keycode in [wx.WXK_RIGHT]:
            self.nextScene(forward=True)
            self.update()
        ...
        if chr(keycode) in "aAdDsS":
            print('lol')
            #self.toggleLightSource(chr(keycode))
            #self.topLevelComponent.initialize()
            self.update()
        if chr(keycode).isnumeric() and chr(keycode) != "0":
            self.scene.toggleLights(chr(keycode))
            #self.topLevelComponent.initialize()
            self.update()
        # TODO 4.2 is at here
        # TODO 5.3 is at here

    def nextScene(self, forward):
        if forward:
            self.currentScene = (self.currentScene + 1) % self.numScenes
        else:
            if self.currentScene == 0:
                self.currentScene = self.numScenes - 1
            else: 
                self.currentScene -= 1
        if self.currentScene == 0:
            self.switchScene(SceneOne(self.shaderProg))
    ...
    def toggleLightSource(self, keycode):
        if keycode in "aA":
            print("here")
